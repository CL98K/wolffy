...
(defun perform-op (op-code &optional stream obj)
  (declare (optimize (speed 3) (safety 0) (debug 0) (compilation-speed 3))
           (type fixnum op-code) (type wo-io:binary-stream stream) (type (or fixnum nil) obj))

  (if (gethash op-code *op-func*)
      (funcall (the function (gethash op-code *op-func*)) stream obj)
      (error 'unpickling-error :code op-code :error "Not implemented")))

(defmacro defop (name (&optional stream obj) &body body)
  (let ((s (or stream (gensym)))
        (o (or obj (gensym))))
    `(setf (gethash ,name *op-func*) #'(lambda (,s ,o) ,(cond
                                                          ((and (not stream) (not obj)) `(declare (optimize (speed 3) (safety 0) (debug 0) (compilation-speed 3)) (ignore ,s ,o)))
                                                          ((not stream) `(declare (optimize (speed 3) (safety 0) (debug 0) (compilation-speed 3)) (ignore ,s)))
                                                          ((not obj) `(declare (optimize (speed 3) (safety 0) (debug 0) (compilation-speed 3)) (ignore ,o))))
                                         ,@body))))

(macrolet ((type-to-code (obj)
             (typecase obj
               (nil     1000)
               (boolean 1001)
               (integer 1010)
  ...
  (add-op +pop+              #\0  "discard topmost stack item")3
